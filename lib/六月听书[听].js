...
var rule = {
    ...
    limit: 6,
    图片来源: '@Referer=http://m.5weiting.com/@User-Agent=MOBILE_UA',
    推荐: '*',
    ...
    搜索: 'js:let d=[];let MY_HOME="http://m.5weiting.com";let html=request(input);html=JSON.parse(html);let list=html.data.content;list.forEach(function(it){d.push({title:it.name.replace(/<.*?>/g,""),img:"http://img.6yueting.com:20001/"+it.coverUrlLocal,url:MY_HOME+"/list/"+it.code,desc:it.cdate,content:it.descXx.replace(/&nbsp;/g,"")})});setResult(d);',
}