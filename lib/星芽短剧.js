...
async function init(cfg) {
  try {
    const data = { 'device': '242506832wEedY4qWiT9to1EZCFXmw48e63XVy6oW' };
    const options = {
      method: 'POST',
      headers: {
...
async function category(tid, pg, filter, extend) {
  ...
  const area = filter && filter.area ? filter.area : ruleFilterDef.星芽.area;
  const headers = xingya_headers.authorization ? xingya_headers : aggConfig.headers.default;
...
async function detail(id) {
  ...
  const headers = xingya_headers.authorization ? xingya_headers : aggConfig.headers.default;
  const res = JSON.parse(await request(realUrl, { headers }));
  const data = res?.data || {};

  const urls = (data.theaters || []).map(
    it => `${it.num}$${it.son_video_url}`
  );
...
async function search(wd, quick, pg) {
  ...
  const res = JSON.parse(
    await request(`${plat.host}${plat.search}`, {
      method: 'POST',
      headers,
      body
    })
  ...
  if (res?.data?.theater?.search_data) {
    res.data.theater.search_data.forEach(it => {
      const id = `${plat.host}${plat.url2}?theater_parent_id=${it.id}`;
      videos.push({
        vod_id: `星芽@${id}`,
        vod_name: it.title,
        vod_pic: it.cover_url,
        vod_remarks: `${it.total || 0}集`,
        vod_content: it.introduction || ''
      });
    });
  }
...
async function play(flag, id, flags) {
  if (/星芽/.test(flag)) {
    try {
      const headers = xingya_headers.authorization
        ? xingya_headers
        : aggConfig.headers.default;

      const res = JSON.parse(await request(id, { headers }));
      const data = res?.data || {};
