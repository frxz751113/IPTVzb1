var rule = {
    ...
    filter: {
      ...
      "50":[{"key":"cateId","name":"分类","value":[{"n":"全部","v":"50"},{"n":"首推","v":"51"},{"n":"社会","v":"52"},{"n":"历史","v":"53"},{"n":"科技","v":"54"},{"n":"自然","v":"55"},{"n":"美食","v":"56"}]},{"key":"sort","name":"排序","value":[{"n":"热门","v":"hot"},{"n":"评分","v":"rating"},{"n":"更新","v":"update"}]}],
      ...
      //"99":[{"key":"cateId","name":"分类","value":[{"n":"全部","v":"99"},{"n":"小编首推","v":"12"},{"n":"近期热映","v":"2"},{"n":"动作大片","v":"16"},{"n":"华语热门电影","v":"8"},{"n":"精彩日韩影片","v":"14"},{"n":"精选港台影片","v":"15"},{"n":"欧美精选影片","v":"9"},{"n":"泰国影片鉴赏","v":"10"},{"n":"豆瓣TOP榜","v":"3"}]}]
    },
    filter_url: 'area={{fl.area or "0"}}&sort={{fl.sort or "update"}}&year={{fl.year or "0"}}&category_id={{fl.cateId}}',
    filter_def: {1: {cateId: '1'},2: {cateId: '2'},3: {cateId: '3'},4: {cateId: '4'},50: {cateId: '50'},67: {cateId: '67'}},
    headers: {
    ...
    lazy: '',
    预处理: function() {
        this.imghost = 'https://img.jgsfnl.com'; 
        try {
            const configUrl = this.img_host + '/api/v2/settings/resourceDomainConfig';
            const response = request(configUrl, {headers: this.headers});
            const res = JSON.parse(response);
            if (res && res.code === 1 && res.data && res.data.imgDomain) {
                const domains = res.data.imgDomain.split(',');
                if (domains && domains.length > 0) {
                    this.imghost = 'https://' + domains[0].trim();
                }
            }
        } catch (e) {
            console.log('获取动态图片域名失败，使用默认域名：' + this.imghost);
        }
        
        console.log('当前图片域名：' + this.imghost);
    },
    推荐: $js.toString(() => {
        rule.预处理();        
...
if (playSources.length > 0) {
    ...
    // ✅ 指定只保留的线路（按你要求的顺序）------------------------------------------------------删除这两条线段之间的内容，就恢复默认线路了。
    const allowNames = [
        '高清线路1',
        '高清线路2',
        '高清线路3',
    ...
    // ✅ 筛选线路
    playSources = playSources.filter(s =>
        allowNames.includes(s.name)
    );

    // ✅ 防止全部被过滤掉（保险）
    if (playSources.length === 0) {
        playSources = [playSources[0]];
    }
