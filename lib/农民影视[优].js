var rule = {
    ...
    */
    ...
    lazy: $js.toString(() => {
        ...
        let mac_from = html.match(/mac_from='(.*?)'/)[1];
        let index = parseInt(input.match(/num-(\d+)/)[1]) - 1;
        let srcIndex = parseInt(input.match(/src-(\d+)/)[1]) - 1;
        let lines = mac_url.split('$$$');
        let playUrls = lines[srcIndex].split('#');
        let playUrl = playUrls[index].split('$')[1];
        let playerHtml = request('https://api.wwgz.cn:520/player/?url=' + playUrl,{headers: {'Referer': 'https://vip.wwgz.cn:5200/'}});
        let m3u8Match = playerHtml.match(/"url":\s*"([^"]+)"/);
        if (m3u8Match) input = {parse: 0,url: m3u8Match[1]};
    }),
    /*lazy: $js.toString(() => {
        let html = request(input);
        let mac_url = html.match(/mac_url='(.*?)';/)[1];
        ...
        let playUrls = lines[srcIndex].split('#');
        let playUrl = playUrls[index].split('$')[1];
        let fromList = mac_from.split('$$$');
        let fromKey = fromList[srcIndex];
        if (fromKey.includes('m3u8')) {
            let playerUrl = 'https://api.wwgz.cn:520/player/?url=' + playUrl;
            let playerHtml = request(playerUrl, {headers: {'Referer': 'https://vip.wwgz.cn:5200/'}});
            let m3u8Match = playerHtml.match(/"url":\s*"([^"]+)"/);
            if (m3u8Match) input = {parse: 0,url: m3u8Match[1]};
        } else {
            let playerUrl = 'https://api.nmvod.me:520/player/?url=' + playUrl;
            let playerHtml = request(playerUrl, {headers: {'Referer': 'https://vip.wwgz.cn:5200/'}});
            let m3u8Match = playerHtml.match(/"url":\s*"([^"]+)"/);
            if (m3u8Match) input = {parse: 0,url: m3u8Match[1]};}
    }),*/
